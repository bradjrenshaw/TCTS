...
const GeneralSettingsTab = ({service}: {service: OutputService}) => {
    ...
    <form>
        ...
        <input id="inputName" type="text" onChange={handleNameChange} value={serviceName} />
    </form>
    </>
};

const ServiceSettings = ({service, onServiceProviderChange}: {service: OutputService, onServiceProviderChange: (serviceProvider: OutputServiceProvider|null) => void}) => {
    let [ serviceProviderName, setServiceProviderName ] = useState<string|undefined>(service.serviceProvider? service.serviceProvider.name : undefined);
    ...
    useLifecycle(() => {
        ...
        let [name, provider] = providerRegistry.getDefaultOutputServiceProvider();
        if (!provider) return;
    ...
    const setOutputServiceProvider = (name: string | undefined) => {
        if (!name) return;
        let provider: typeof OutputServiceProvider = providerRegistry.outputServiceProviders[name];
        if (!provider) {
    ...
    return <>
    <form>
    <select onChange={handleServiceNameChange} value={serviceProviderName}>
    {Object.keys(providerRegistry.outputServiceProviders).map((name) => <option key={name} value={name}>{name}</option>)}
    </select>
    </form>
    {service.serviceProvider ? <service.serviceProvider.ServiceSettingsComponent /> : <NoServiceProvidersError />}
    </>;
};

const OutputSettings = ({serviceProvider}: {serviceProvider: OutputServiceProvider | null}) => {
    if (serviceProvider) {
...
const OutputServiceSettings = ({originalService, editing = false, onConfirm, onCancel}: {originalService: OutputService, editing?: boolean, onConfirm: OnConfirmCallbackType, onCancel: OnCancelCallbackType}) => {
    let [ service, setService ] = useState<OutputService>(originalService);
    let [ serviceProvider, setServiceProvider ] = useState<OutputServiceProvider | null>(null);
    ...
    const handleServiceProviderChange = (serviceProvider: OutputServiceProvider|null) => {
    ...
    <Tabs>
        ...
        <TabPanel>
            <ServiceSettings service={service} onServiceProviderChange={handleServiceProviderChange} />
        </TabPanel>
        <TabPanel>
