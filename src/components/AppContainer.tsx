...
const AppContainer = (props: any) => {
    ...
    let [ tabManager, setTabManager ] = useState<UniqueItemList<Profile>>(new UniqueItemList(profileTabs, false, (profiles: Array<Profile>) => {
        setProfileTabs(profiles);
    }));

        return <TabManagerContext.Provider value={tabManager}>
        <Tabs>
        <TabList>
            <Tab>main</Tab>
            {profileTabs.map((profile: Profile) => <Tab key={profile.name}>{profile.name}</Tab>)}
        </TabList>
        <TabPanel>
            <h1>Profiles</h1>
        <ProfilePanel />
