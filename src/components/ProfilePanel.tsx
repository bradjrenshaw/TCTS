...
const ProfileRow = ({profile, onAction}: {profile: Profile, onAction: any}) => {
...
<td>{profile.name}</td>
        ...
        <td><ChatServiceConnectDisconnectButton service={profile.chatService} /></td>
<td><button onClick={handleDeleteClick}>Delete</button></td>
        </tr>
    );
}

const ProfileList = ({profiles, onAction}: {profiles:Array<Profile>, onAction: any}) => {
    ...
    return (<div><table>
        ...
        <tbody>
        {profiles.map((p: Profile) => <ProfileRow profile={p} key={p.name} onAction = {onAction} />)}
        </tbody>
        <tfoot>
        <tr><td><button onClick={onAddClick}>Add</button></td></tr>
        </tfoot>
    </table></div>);
   }
...
const ProfilePanel = () => {
    ...
    if (action === Action.List) {
        return <div><ProfileList profiles={profiles} onAction = {onAction} /></div>
    }else if (action === Action.Add) {
        return <Modal visible onClose = {handleCancel}>
{actionProfile ? <ProfileSettings originalProfile={actionProfile} onConfirm={handleNewProfileSave} onCancel={handleCancel} /> : <></>}
        </Modal>
    } else if (action === Action.Edit) {
        return <Modal visible onClose = {handleCancel} >
        {actionProfile ? <ProfileSettings editing originalProfile={actionProfile}  onConfirm = {handleEditProfileSave} onCancel = {handleCancel} /> : <></>}
                </Modal>
    } else {
        return <p>Deleting</p>;
    }
