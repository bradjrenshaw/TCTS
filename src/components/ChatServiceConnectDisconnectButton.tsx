...
const ChatServiceConnectDisconnectButton = ({service}: {service: ChatService | null}) => {
    let tabManager = useContext(TabManagerContext);
    let [ connected, setConnected ] = useState(service ? service.isConnected() : false);
    ...
    return <button onClick = {handleClick} disabled={!service || transition}>{connected ? "Disconnect" : "Connect"}</button>
};
