...
export default class DataManager {
    ...
    constructor(providerRegistry: ProviderRegistry) {
        ...
        addProfile(profile: Profile): void {
            for (let p of this.profiles) {
                ...
                throw new Error("A chat profile with the name " + profile.name + " already exists.");
                }
            }
        ...
        private addProfileFromData(data: any): void {
            ...
            if (chatServiceData && Object.keys(chatServiceData).length > 0) {
                let chatServiceType: typeof ChatService = this.providerRegistry.chatServices[chatServiceData.name];
                if (!chatServiceType) {
                    throw Error("Chat service " + chatServiceData.name + " could not be found.");
                }
                profile.chatService = chatServiceType.prototype.deserialize(this, profile, chatServiceData);
            }
        ...
        public addOutputService(service: OutputService) {
            for (let s of this.outputServices) {
                if (s.name === service.name) {
                    throw new Error("An output service with the name " + s.name + " already exists.");
                }
            }
        ...
        public replaceOutputService(original: OutputService, replacement: OutputService): boolean {
    ...
    private serialize(): any {
        return {
            outputServices: this.outputServices.map((service: OutputService) => service.serialize()),
            profiles: this.profiles.map((profile: Profile) => profile.serialize())
        };
    }
