...
export default class ProviderRegistry {
    ...
    public readonly outputServiceProviders: {[key: string]: typeof OutputServiceProvider};
    ...
    getDefaultChatService(): [string|undefined, typeof ChatService|undefined] {
    ...
    registerChatService(name: string, service: typeof ChatService): void {
        ...
        throw new Error("A chat provider with the name " + name + " already exists. Object is " + this.chatServices[name]);
    }
    this.chatServices[name] = service;
}

    getDefaultOutputServiceProvider(): [string | undefined, typeof OutputServiceProvider | undefined] {
        let key: string = Object.keys(this.outputServiceProviders)[0];
        if (!key) {
            return [undefined, undefined];
        }
        let service: typeof OutputServiceProvider = this.outputServiceProviders[key];
        if (!service) {
            return [undefined, undefined];
        }
        return [key, service];
    ...
    registerOutputServiceProvider(name: string, provider: typeof OutputServiceProvider) {
        if (this.outputServiceProviders[name]) {
            throw Error("An output service provider with the name " + name + " already exists.");
        }
