...
export default class UniqueItemList<T> {
    ...
    constructor(items: Array<T>|undefined = undefined, strict: boolean = false, updateCallback: UpdateCallback|null = null) {
    ...
    push(item: T): void {
        for (let i of this.items) {
            if (i === item) {
                if (this.strict) throw new Error("item " + item + " already exists in UniqueItemList " + this);
                return;
            }
        }
    ...
    remove(item: T) {
        ...
        if (index < 0) {
            if (this.strict) throw new Error("Item " + item + " does not exist in UniqueItemList " + this);
            return;
        }
        this.items.splice(index, 1);
        this.items = [...this.items];
        this.length = this.items.length;
        if (this.defaultItem === item) this.defaultItem = this.length > 0 ? this.items[0] : undefined;
        if (this.updateCallback) this.updateCallback(this.items);
    }
    ...
    replace(oldItem: T, newItem: T) {
        ...
        this.items = [...this.items];
        if (this.defaultItem === oldItem) this.defaultItem = this.length > 0 ? this.items[0] : undefined;
        if (this.updateCallback) this.updateCallback(this.items);
        return true;
    }
};
